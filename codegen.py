...
class Context(object):
    ...
    def render(self):
        d = dict(
            classes = [ render_class(c, self) for c in self._classes ],
            enums   = [ render_enum(c, self) for c in self._enums ],
        )
        return d
...
def render_method(m, ctx):
    d = dict(
        ...
        result_type     = m.result_type.spelling,
        arg_names       = [ n.spelling for n in m.get_arguments() ],
        arg_types       = [ n.type.spelling for n in m.get_arguments() ],
...
method_template = '''
        {%- if m.mode == 'short' %}
        {{ m|disabled }}.def("{{m.name}}", ({{ m.signature }})&{{ m.class }}::{{ m.name }})
        {% else %}
...
enum_template = '''
    py::enum_<{{e.typename}}>({{ e.parent|default("m", boolean=True) }}, "{{e.typename|replace("::", "_")}}")
        {% for v in e.xitems %}
...
if __name__ == "__main__":
    ...
    import pprint
    ctx = build_context()
    #print json.dumps(ctx.render(), sort_keys=True, indent=4)
    print render_result(**ctx.render()) 
