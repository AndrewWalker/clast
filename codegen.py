...
def render_result(template, **kwargs):
    ...
    env.filters['fdeleter']  = fdeleter
    env.filters['enum_parent'] = enum_parent
    return env.get_template(template).render(model = kwargs)
...
if __name__ == "__main__":
    ...
    with open(os.path.join(args.output, '00_autogen_enums.cpp'), 'w') as fh:
        fh.write(render_result(template='enum_module.j2', prelude=c_src, **intermediate)) 
    with open(os.path.join(args.output, '01_autogen_classes.cpp'), 'w') as fh:
        fh.write(render_result(template='class_module.j2', prelude=c_src, **intermediate)) 
